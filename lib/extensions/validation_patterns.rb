module ValidationPatterns
  ...
  EmailUser = /[#{email_user_chars}]+(?:\.?[#{email_user_chars}])*|".+?"/iu
  ...
  EmailAddress = /\A(?:#{EmailUser.source})\@(?:(?:#{DomainName.source})|\[(?:#{PublicIPv4Address.source})\])\Z/iux

  URL = Url = %r~\Ahttps?://(?:(?:#{DomainName.source})|(?:#{PublicIPv4Address.source}))(?::\d{2,5})?(?:/[^\s]*)?\Z~iux
end