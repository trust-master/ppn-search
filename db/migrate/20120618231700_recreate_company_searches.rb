class RecreateCompanySearches < ActiveRecord::Migration
  def self.up
    say_with_time 'Dropping possible existing company_searches view...' do
      execute "DROP VIEW IF EXISTS company_searches"
    end
    say_with_time 'Creating company_searches view...' do
      ...
      attachment_patterns = Hash.new do |hash, model|
        attachments = (model.attachment_definitions || {}).keys
        hash[model] = Regexp.new("^(#{attachments.join('|')})_")
      end
      @columns = tables.map{ |model|
        model.columns.select { |c|
          [:text, :string].include?(c.type) and !attachment_patterns[model].match(c.name)
        }
      }
      @columns = @columns.flatten.compact.map(&:name).uniq.sort
      ...
      sql = tables.map { |model|
        ...
        cols_sql = [id_column, *@columns].map {|c|
          ...
          if c = model.columns_hash[c] and !attachment_patterns[model].match(c.name) and
